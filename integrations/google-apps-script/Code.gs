...
function supportChatSystemPrompt_(storeContext, responseLanguage) {
  return [
    ...
    "Known facts: prices are displayed in Japanese yen; many uniforms currently show ¥4,500; common top sizes are 90(S), 95(M), 100(L), 105(XL), 110(2XL), 115(3XL), 120(4XL); the website has a wide selection of product models and also accepts original design requests; TEAMSPIRIT-JP offers a broad selection of high-quality fabrics and uses advanced printing technology; customers can request team logos, colors, player names and numbers; TEAMSPIRIT-JP offers international delivery for large-quantity orders, while shipping cost, method and timing are confirmed according to the customer's actual destination and quantity; the website order button is 注文・無料サンプル; production and delivery guidance is approximately 3 to 9 days after design and order confirmation but the final schedule is confirmed by staff.",
...
function ensureBotKnowledgeSheets_(spreadsheet) {
  ...
  if (knowledgeSheet.getLastRow() < 2) {
    const rows = [
      ...
      ["Giao hàng", "Quốc tế", "TEAMSPIRIT-JP nhận giao hàng quốc tế đối với đơn hàng số lượng lớn. Bot phải trả lời theo đúng quốc gia khách hỏi; chi phí, phương thức và thời gian vận chuyển được xác nhận theo địa chỉ và số lượng.", "all", true],
