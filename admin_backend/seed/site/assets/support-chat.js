(() => {
  ...
  function guidedSalesAnswer(question) {
    ...
    if (/(?:giao hàng|giao hang|vận chuyển|van chuyen|ship|shipping|delivery|international|quốc tế|quoc te|việt nam|viet nam|海外配送|国際配送|発送|届け)/i.test(text)) {
      const viMatch=text.match(/(?:giao hàng|giao hang|ship)(?:\s+(?:đến|tới|về|den|toi|ve))?\s+([^?!.]+)/i),enMatch=text.match(/(?:ship|shipping|deliver|delivery)(?:\s+(?:to|in))?\s+([^?!.]+)/i);
      const cleanDestination=value=>String(value||"").replace(/\s+(?:không|khong|ko|được không|duoc khong|không ạ|khong a|available).*$/i,"").replace(/^(?:hàng|hang)\s+/i,"").trim();
      const destination=cleanDestination(lang==="vi"?viMatch?.[1]:lang==="en"?enMatch?.[1]:"");
      if(lang==="vi") return destination?`Có nhé. TEAMSPIRIT-JP có thể giao hàng đến ${destination} cho các đơn hàng số lượng lớn. Chi phí, phương thức vận chuyển và thời gian giao hàng sẽ được nhân viên xác nhận theo địa chỉ và số lượng. Bạn hãy bấm “Đặt hàng/Mẫu thử miễn phí” hoặc liên hệ LINE để được tư vấn chi tiết nhé.`:"Có nhé. TEAMSPIRIT-JP nhận giao hàng quốc tế cho các đơn hàng số lượng lớn. Chi phí, phương thức vận chuyển và thời gian giao hàng sẽ được xác nhận theo quốc gia, địa chỉ và số lượng. Bạn hãy bấm “Đặt hàng/Mẫu thử miễn phí” hoặc liên hệ LINE để được tư vấn chi tiết nhé.";
      if(lang==="en") return destination?`Yes. TEAMSPIRIT-JP can deliver large-quantity orders to ${destination}. Shipping cost, method and delivery time are confirmed according to the address and order quantity. Please use the Order/Free Sample button or contact us on LINE for details.`:"Yes. TEAMSPIRIT-JP offers international delivery for large-quantity orders. Shipping cost, method and delivery time are confirmed according to the destination and quantity. Please use the Order/Free Sample button or contact us on LINE for details.";
      return "はい。TEAMSPIRIT-JPでは、大口注文を対象に海外配送に対応しています。送料・配送方法・お届け日数は、配送先の国・住所・注文数量に応じて担当者が確認します。「注文・無料サンプル」ボタン、またはLINEから詳細をご相談ください。";
    }
  ...
  function mount() {
    ...
    let largestSupportViewportHeight=window.visualViewport?.height||window.innerHeight;
    function positionSupportUi(){if(!matchMedia("(max-width:600px)").matches){document.documentElement.classList.remove("ts-support-keyboard");[launcher,panel].forEach(element=>["left","right","top","bottom"].forEach(name=>element.style.removeProperty(name)));return}const viewport=window.visualViewport,width=viewport?.width||innerWidth,height=viewport?.height||innerHeight,left=viewport?.offsetLeft||0,top=viewport?.offsetTop||0;largestSupportViewportHeight=Math.max(largestSupportViewportHeight,height);const keyboardOpen=height<largestSupportViewportHeight*.78;document.documentElement.classList.toggle("ts-support-keyboard",keyboardOpen);const launcherWidth=launcher.offsetWidth||112,launcherHeight=launcher.offsetHeight||40,launcherLift=panel.hidden?54:8,launcherLeft=Math.max(left+6,left+width-launcherWidth-8),launcherTop=Math.max(top+6,top+height-launcherHeight-launcherLift);launcher.style.setProperty("left",`${launcherLeft}px`,"important");launcher.style.setProperty("right","auto","important");launcher.style.setProperty("top",`${launcherTop}px`,"important");launcher.style.setProperty("bottom","auto","important");if(panel.hidden)return;const panelWidth=Math.min(width-20,500),panelHeight=panel.getBoundingClientRect().height,panelLeft=left+Math.max(10,(width-panelWidth)/2),panelTop=Math.max(top+6,launcherTop-panelHeight-8);panel.style.setProperty("width",`${panelWidth}px`,"important");panel.style.setProperty("left",`${panelLeft}px`,"important");panel.style.setProperty("right","auto","important");panel.style.setProperty("top",`${panelTop}px`,"important");panel.style.setProperty("bottom","auto","important")}
    window.positionTeamspiritSupport=()=>requestAnimationFrame(positionSupportUi);window.addEventListener("resize",window.positionTeamspiritSupport,{passive:true});window.addEventListener("orientationchange",()=>setTimeout(window.positionTeamspiritSupport,120),{passive:true});window.visualViewport?.addEventListener("resize",window.positionTeamspiritSupport,{passive:true});window.visualViewport?.addEventListener("scroll",window.positionTeamspiritSupport,{passive:true});
    function render() { messages.innerHTML=history.map(item=>`<div class="ts-support-message ${item.role}"><small>${item.role === "user" ? "お客様" : "サポート"}</small><p>${esc(item.text)}</p></div>`).join(""); messages.scrollTop=messages.scrollHeight;window.positionTeamspiritSupport(); }
    ...
    function toggle(force){const open=typeof force==="boolean"?force:panel.hidden;panel.hidden=!open;launcher.setAttribute("aria-expanded",String(open));document.documentElement.classList.toggle("ts-support-open",open);if(open){render();input.focus()}window.positionTeamspiritSupport()}
