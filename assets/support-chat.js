(() => {
  ...
  function mount() {
    ...
    function positionSupportUi(){if(!matchMedia("(max-width:600px)").matches){[launcher,panel].forEach(element=>["left","right","top","bottom"].forEach(name=>element.style.removeProperty(name)));return}const viewport=window.visualViewport,width=viewport?.width||innerWidth,height=viewport?.height||innerHeight,left=viewport?.offsetLeft||0,top=viewport?.offsetTop||0,launcherWidth=launcher.offsetWidth||96,launcherHeight=launcher.offsetHeight||39,launcherLeft=Math.max(left+6,left+width-launcherWidth-8),launcherTop=Math.max(top+6,top+height-launcherHeight-8);launcher.style.setProperty("left",`${launcherLeft}px`,"important");launcher.style.setProperty("right","auto","important");launcher.style.setProperty("top",`${launcherTop}px`,"important");launcher.style.setProperty("bottom","auto","important");if(panel.hidden)return;const panelWidth=Math.min(width-20,500),panelHeight=panel.getBoundingClientRect().height,panelLeft=left+Math.max(10,(width-panelWidth)/2),panelTop=Math.max(top+8,launcherTop-panelHeight-10);panel.style.setProperty("width",`${panelWidth}px`,"important");panel.style.setProperty("left",`${panelLeft}px`,"important");panel.style.setProperty("right","auto","important");panel.style.setProperty("top",`${panelTop}px`,"important");panel.style.setProperty("bottom","auto","important")}
