...
const context = {
  ...
  UrlFetchApp: { fetch: () => { aiRequests += 1; return { getResponseCode: () => 200, getContentText: () => JSON.stringify({ choices: [{ message: { content: "担当者に確認します。" } }] }) }; } },
...
const chatResponse = JSON.parse(context.doPost({ postData: { contents: JSON.stringify({ action: "support_chat", question: "大会用の注文について相談できますか？", context: { title: "商品ページ" } }) } }).text);
